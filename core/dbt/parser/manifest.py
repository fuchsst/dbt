...
class ManifestLoader:
    ...
    def get_full_manifest(
        ...
        *,
        ...
        with PARSING_STATE:
            ...
            loader.load()
            # The goal is to move this to after update_manifest
    ...
    def parse_project(
        ...
        # so do separately
        parser = HookParser(project, self.manifest, self.root_project)
        path = parser.search()[0]
        file_block = FileBlock(load_source_file(path, ParseFileType.Hook, project.project_name))
        self.parse_with_cache(file_block, parser)
